...
4. **Ownership-manifest version transition (verify pass-2 fix).** Pass 2 tightened the pass-1 BLOCKER fix: old manifest records must pass their exact prior-version validation and exact live-symlink checks, may be used only to retire those legacy links, and must not seed new ownership; desired token data derives every new link and manifest record. Verify this permits an atomic upgrade and rollback without orphaning old links or weakening corrupt-state refusal (`archetypes/modules/pi-provider-reconcile.sh`, `archetypes/checks/pi-provider-lifecycle.nix`).
