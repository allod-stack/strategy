...
4. **Ownership-manifest version transition (verify pass-3 fix).** Pass 3 added the missing piece: legacy per-credential links and new per-credential token directories are pinned to the identical leaf path (`<credential-link-root>/<credential>`), so retiring the legacy symlink must be sequenced strictly before creating the new directory there, with any non-legacy occupant refusing the whole operation. Verify this ordering is actually enforceable as a single atomic reconciliation (not two separate transactions), that the `rmdir`/`mkdir` sequencing at one path cannot leave a state with neither a link nor a directory if interrupted mid-conversion, and that pass-2's exact prior-version validation, live-symlink checks, and desired-only new-ownership derivation still hold given the added same-path constraint (`archetypes/modules/pi-provider-reconcile.sh`, `archetypes/checks/pi-provider-lifecycle.nix`).
