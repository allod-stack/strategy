...
- The framework declares which guest mount points an archetype must keep. For the dev archetype this includes the user's home, which contains checkouts, in-flight work, and agent state, and one Nix state volume mounted at `/nix/var/nix`; contract 6a uses that same volume for Nix's database, profiles, gcroots, writable store overlay upper/work directories, and the small per-toplevel registration copies needed to reconcile a changing read-only lower store and replay a rollback.
- Deployment data supplies host placement through the guest's own `microvm.volumes` entries, which is the only place it can live: that image string is what QEMU renders into `-drive file=` and what the root launch helper preflights. There is deliberately no second host-side placement option to keep in sync with it. Initial image creation, filesystem formatting, ownership, and capacity are explicit private provisioning actions before a VM is enabled; the framework does not create volumes on start.
...
1b. **The fleet starts what inventory declares.** Every machine whose inventory entry selects the microvm runtime appears in exactly one exported hypervisor's guest set, and no hypervisor starts a machine inventory did not select. Which guests a hypervisor composes stays a builder argument in Archetypes, so a check can drive the builder with a guest set no inventory supplies; the rule is asserted on the `nixosConfigurations` output instead, because that is what a deployment consumes. This closes the seam where per-machine builder overrides — `profileData` or a `profile` setting — hand a hypervisor an empty guest set and silently un-wire a declared machine while every other check stays green, a failure a byte-identical derivation comparison cannot distinguish from a working gate. Exactly one host, not at least one: nothing in inventory assigns a guest to a host, so the framework default gives every hypervisor the whole declared set minus itself, and a fleet with two hypervisors would wire each guest into both — two hosts sharing one VM's state directory and QMP socket. Such a fleet is refused by name rather than shipped, and names the host it means through each hypervisor's guest argument.
...
6a. **A selected dev guest keeps one coherent writable Nix store.** At the pinned revision the upstream guest module mounts `/nix/store` straight from the read-only erofs store disk, and sets `systemd.services.nix-daemon.enable` and its socket false, whenever `microvm.writableStoreOverlay` is null. A guest that sets only `storeOnDisk` therefore cannot realise, fetch, or collect a store path, while still carrying the shared guest base's flake settings and trusted users and contract 9's Nix netrc and Git credential wiring — a dev VM that cannot run `nix build`, `nix flake check`, or `nix flake update` does not do the job the archetype exists for. The upstream overlay option is not complete state by itself: when the overlay upper/work directories live on a persistent mount outside `/nix/var/nix`, the store files can survive a reboot while `/nix/var/nix` and its Nix database, profiles, and gcroots revert with the tmpfs root, so Nix forgets the built paths. Every selected dev microvm therefore declares one persistent Nix state volume at `/nix/var/nix` and sets `microvm.writableStoreOverlay = "/nix/var/nix"`. That makes `/nix/store` an overlay of the erofs lower plus upperdir `/nix/var/nix/store` and workdir `/nix/var/nix/work`, keeps the Nix database, profiles, gcroots, and overlay backing in the same `neededForBoot` volume, and re-enables the daemon.

The pinned upstream `microvm.registerClosure` path is deliberately disabled for this persistent overlay. Its registration file is an extra path on the read-only store disk but is not registered by its own contents, so guest garbage collection removes it by creating a persistent overlay whiteout. Independently, after the host moves the guest toplevel, the persistent database continues to call paths from the previous erofs lower valid even though they are physically absent; `nix path-info` can then succeed while `nix-store --verify-path` fails. A later `nix-store --verify` removes those stale entries, but rollback cannot re-register the old lower if its registration file was already whiteouted. The framework therefore sets `microvm.registerClosure = false`, reuses the registration derivation exposed by the generated `microvm.storeDisk`, passes its path only through the runner's kernel command line — with the string context discarded exactly as upstream discards it for the `init=` argument, because the payload is already inside the erofs disk and a context-bearing reference would pull the closure-info derivation's whole-guest-closure reference graph into every host runner closure — and owns one persistent-state-safe pre-systemd replay. On first boot of a toplevel, that replay validates the kernel argument and atomically copies the lower store's registration file to a root-owned path keyed by that registration derivation under `/nix/var/nix/microvm-registrations/` before any GC can hide it. On every boot it requires either that exact lower file or its exact cached copy, reconciles the persistent database against the current merged store, loads the current registration copy, and proves the current toplevel with `nix-store --verify-path` or `nix path-info` before the Nix daemon or any dev session may start. One `nix-store --verify` run is not that reconciliation: at the pinned Nix 2.31, `--verify` removes an absent path's database entry only while no surviving valid path references it; an absent path that still has a valid referrer is kept, reported as `disappeared, but it still has valid referrers!`, and makes verify exit 1 on that and every later run. After an ordinary toplevel move that referenced-absent state is the common one, not the corrupt one, because paths built into the persistent upper store under the previous toplevel reference old lower-only paths — the dev guest builds against the same pinned nixpkgs its own system closure comes from. Reconciliation therefore invalidates every database entry whose path is absent from the merged store together with the valid entries whose reference closures reach one, leaving surviving files on disk as unregistered upper data rather than deleting them, so that a repeated whole-store verify is clean and expected post-rebuild divergence is never the terminating case. A same-toplevel restart after GC and a rollback whose lower registration file is already whiteouted both use the cached copy. A malformed kernel path, a missing or corrupt current registration source and cache, reconciliation failure, load failure, or invalid current toplevel terminates the boot before systemd consumers rather than relying on `boot.postBootCommands`' ignored exit status.

`nix.optimise.automatic` and `nix.settings.auto-optimise-store` stay false, as upstream asserts for that overlay. `microvm.storeOnDisk` still resolves true and `microvm.shares` stays empty, so contract 6 is unchanged, and contracts 13 and 14 govern the Nix state volume like any other persistent mount. A selected dev microvm with a null `writableStoreOverlay`, a value other than `/nix/var/nix`, no declared persistent `/nix/var/nix` volume, upstream `microvm.registerClosure` enabled, no framework registration replay, or store auto-optimisation enabled fails evaluation.

Contract 6a is an empirical specification, recorded as such deliberately: the overlay came from review pass 7, the database volume from pass 8, the registration replay from pass 9, and the reconciliation semantics from pass 10, each correcting the last from the pinned sources and store-level measurement, and no pass has executed the assembled lifecycle in a booted guest. The named store-lifecycle boots in acceptance test 9 — fresh volume, GC whiteout with a same-toplevel restart, a toplevel move with and without a surviving upper referrer into the old lower, A-to-B-to-A rollback, cache sabotage, and replay termination — are the evidence that settles this contract, and the archetypes milestone that first selects a dev microvm does not land until they pass in the real nested boot. Where an implementation measurement contradicts a sentence here, the measurement wins: update this contract through the normal change process rather than working around it, and never by silently re-enabling upstream `microvm.registerClosure`.
...
8b. **The host option crosses the guest-evaluation boundary once.** A standalone Archetypes microvm configuration declares the required credential-name set but no host root or `credentialFiles` value. When Archetypes composes the hypervisor, it uses pinned nixpkgs's `extendModules` interface on each selected exported guest `nixosConfiguration` to add one module whose `microvm.credentialFiles` map derives from the enclosing host's evaluated Nexus plaintext-root option and that guest's declared names, then assigns that extended result to upstream `microvm.vms.<name>.evaluatedConfig`. The runner installed and started by the host is built from that same extended result. Nexus does not import Archetypes or duplicate the name set, and the standalone unextended guest output is not accepted as the host runner. This keeps dependency direction intact while making a host-root override change the actual QEMU `-fw_cfg file=` arguments. Supplying `evaluatedConfig` rather than `config` also sets upstream's `microvm.vms.<name>.restartIfChanged` default to false, because that default keys off `microvm.vms.<name>.config != null`. That default is kept, deliberately. A microvm guest is its own deploy target the way this fleet's libvirt VMs are: `rebuild-vm-from-host` switches a guest in place and its services restart at service granularity, which is what contract 6a's writable store overlay and nix daemon exist for. A host rebuild therefore re-runs `install-microvm-<name>` to stage the changed guest's runner and leaves the running guest alone; the next start picks it up. No selected microvm sets `restartIfChanged`. The check pins the rendered `X-RestartIfChanged=false` and the absence of `X-RestartIfChanged=true` rather than the option that produced them, because `switch-to-configuration` treats a missing key as true — so an upstream default change, or an upstream that stopped emitting the key, goes red instead of quietly making every guest restartable on the next host rebuild.
...
17. **Host and guest declarations agree before boot.** For each selected microvm, the prepared host credential names equal the extended guest's `credentialFiles` keys, every value equals the exact host-option-derived `active/<machine>/<credential-name>` path from contract 7, the host installs the runner from that extended guest rather than the standalone base result, and the runtime in host tooling equals inventory and `vmFacts`. Volume images are not part of this agreement because they have only one declaration to agree with: the launcher preflights the images it reads off the guest, so a host-side copy could only ever be duplication kept in sync by an assertion, or a value the guest derives and is then compared against itself. The generated check evaluates and compares the two sides; it does not trust duplicated prose or a default-path text scan.
...
9. Realise or add two paths into the guest's Nix store and root one beneath `/nix/var/nix/gcroots`. On the fresh Nix state volume, assert the pre-systemd replay caches and loads the current lower-store registration, both paths are valid, the nix daemon is running, `/nix/var/nix` is a declared `neededForBoot` persistent volume, and `/nix/store` is an overlay whose upper/work directories are under `/nix/var/nix`. Run guest garbage collection and assert the rooted path remains byte-valid, the unrooted path is collected, and the lower registration file is now hidden; restart the same toplevel and require the cached registration plus rooted path to remain valid. Before changing the toplevel, realise one additional rooted upper path that references a path present only in the current lower store, so the coming move strands an absent path behind a surviving valid referrer. Then change the guest toplevel used by the generated restart trigger and restart: before reconciliation, demonstrate that at least one old lower-only path is physically absent while the persistent database still reports it valid; after reconciliation, require every such stale query to fail, a repeated `nix-store --verify` to exit clean, the boot to proceed rather than terminate on this expected divergence, and the new current toplevel plus the reference-complete rooted path to pass `nix-store --verify-path` or `nix path-info`. Assert the measured fate of the lower-referencing survivor explicitly — invalidated, files retained until guest GC — rather than letting it fall through. Run `nix-store --verify` and GC under the new toplevel, roll back to the old toplevel whose lower registration is whiteouted, and require its cached registration to restore a valid current closure without losing the reference-complete rooted upper path; record what remains of the lower-referencing one rather than asserting its survival. Deleting or corrupting the current cached copy after its lower source is whiteouted must terminate before systemd consumers. Paired fixtures must also prove `writableStoreOverlay = null` fails the write against the read-only erofs mount; an overlay backing persisted outside `/nix/var/nix` can leave the file in `/nix/store` but fails Nix validity after reboot because the database was lost; and a missing, unformatted, or mislabeled `/nix/var/nix` volume fails in the initrd before writers can use the tmpfs directory underneath.
10. Start cold with no prepared credential or QMP directory. Exercise a preparation failure, an early QEMU failure with `Restart=always`, manual stop and restart, and rollback restart. In every path assert the final Nexus `ExecStopPost` runs after the rendered upstream post-stop commands, removes any complete or partial credential and QMP directories only after QEMU exits, preserves the volume and rollback slot, and lets the next attempt recreate a different QMP-directory inode with a newly reachable socket. For the successful stop, install a guest shutdown unit that writes a unique sentinel to the persistent volume, ordered after that mount and before `shutdown.target` so it lands while the volume is still mounted; invoke the rendered upstream QMP `ExecStop` and require that sentinel before accepting directory cleanup as a graceful shutdown. The pinned stop path sends ctrl-alt-del over QMP and the runner carries `-no-reboot`, so the guest runs a reboot transaction that QEMU turns into an exit: the sentinel must survive that path, not only a poweroff. In a separate sabotage case remove or break the socket while QEMU is live, stop the unit, and prove the sentinel is absent when the pinned shutdown script falls through and systemd kills QEMU; directory removal or a successful later restart alone does not count as QMP-shutdown evidence.
11. Rebuild the host with a guest change that moves that guest's `system.build.toplevel`, and assert the running VM is left alone: the `microvm@<name>` unit keeps its main PID and its start timestamp across `switch-to-configuration`, while `install-microvm-<name>` re-runs and its `current` symlink comes to point at the new runner. Then stop and start the unit and assert the changed guest is what boots. This is the behavioural half of contract 8b's restart decision — the drop-in read proves the flag, and only a rebuild on a real host proves what the flag buys. Assert it against a guest change that a rebuild would have restarted under `X-RestartIfChanged=true`, so a fixture that simply failed to change anything cannot pass it.
...
- Selected dev microvm with a null `writableStoreOverlay`, a value other than `/nix/var/nix`, no declared persistent `/nix/var/nix` volume, upstream `microvm.registerClosure` enabled, no framework registration replay, or store auto-optimisation enabled.
...
- Each selected microvm's rendered `microvm@<name>` drop-in carries `X-RestartIfChanged=false` and does not carry `X-RestartIfChanged=true`, so a host rebuild stages a changed guest without stopping the running one. The absence is asserted beside the value because `switch-to-configuration` treats a missing key as true, which makes an upstream that stopped emitting the key as bad as one that flipped its default.
- Each selected dev microvm has `microvm.writableStoreOverlay = "/nix/var/nix"`, `/nix/var/nix` as a declared `neededForBoot` persistent volume, `/nix/store` overlay upper/work below that volume, upstream `microvm.registerClosure = false`, nix daemon and socket enabled, and store auto-optimisation disabled. Its runner carries the registration path exposed by its generated store disk; the pre-systemd replay atomically caches that exact payload under `/nix/var/nix`, reconciles the persistent database to the current merged store before loading it — converging on test 9's referenced-absent divergence rather than terminating on it — verifies the current toplevel, and has paired missing/corrupt-cache sabotage that cannot fall through to systemd.
...
A NixOS test starts two credentialed guests under their own per-VM principals and inspects their real namespace mount tables. From each unit's namespace it uses direct paths, symlinks, alternate bind paths, an inherited working directory, and `/proc/<sibling-pid>/root` to attack the sibling's credential, its `/var/lib/microvms/<name>` entries including `current` and `booted`, its QMP directory and socket, its volume image, and each host source parent, and it signals the sibling's runner. Credential reads; runner and `booted` reads, execution, and replacement; state-directory entry creation, rename, and deletion; source-parent traversal; QMP connect, unlink, rename, and replacement; image read, write, truncate, rename, and replacement; and signal delivery all fail, while each unit retains exactly the required access to its own objects. Collapsing both guests onto one shared runner principal, with the namespace left intact, must reopen the `/proc/<sibling-pid>/root` and signal attacks; a fixture that proves only the direct-path half does not satisfy this check, and narrowing the procfs case is the specific outcome that would close it with the boundary still open. The namespace is attributed differently, because implementation measured that removing it does not reopen the direct-path attacks but stops the VM starting at all: every source parent the launcher creates is root-only, so a runner without the namespace cannot reach its own credential or QMP directory either. That measured outcome is itself pinned by a sabotage node, and the direct-path attribution is made non-destructively by running the same matrix from the same principal inside and outside the namespace on the working configuration. Each case must record which control denies it, so that a case denied by a root-only parent is never credited to the namespace.
...
For a private guest that has already run, stop microvm autostart and return the inventory selection to libvirt only after relaying or pushing all work from every declared persistent volume. For dev microvms, `/nix/var/nix` is part of that authoritative set: its database, profiles, gcroots, overlay upper/work directories, and cached per-toplevel registrations move together, and preserving only home or only the store overlay files is not a valid relay. Before accepting a microvm toplevel rollback, require the contract-6a replay to reconcile the persistent database, load that toplevel's cached registration when the lower copy is whiteouted, and verify the rolled-back current system plus a reference-complete rooted upper-store canary. A revert reconstructs the root and store image but cannot reconstruct unique files on a volume.
