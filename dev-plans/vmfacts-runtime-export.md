...
4. **The coherence check must be able to fail, and today's cannot.** The existing projections enumerate fields — `{ ip: .value.ip, forge_key: .value.forgeKey }` at `flake.nix:980-988` — so a new key is projected away and passes unverified. Add `runtime` to both sides of that projection so `vmFacts` is diffed against `${inventory}/scripts/vm-specs.json`, which already carries it (`inventory/scripts/vm-specs.json:20`, `:31`, both `"libvirt"`).
   ...
   - The negative invocation is then **pinned to its reason**, and pinning on the comparator's own ERROR string is *not enough* — that message is emitted for divergence in any projected field, and is also emitted when the candidate-side `jq` fails, because errexit is suppressed inside a function called in an `if` condition. Measured: a name-set mismatch, a malformed candidate fixture, and drift on `ip` alone each satisfy that grep with `runtime` untouched. So `drift.log` must additionally be grepped for the **runtime line of the diff itself** — `'^> *"runtime": "<driftValue>"'`, where `>` is the facts side of `diff inventory-specs.json facts-specs.json`.
...
2. **The coherence comparison detects runtime drift, for the right reason** — contract 4's negative invocation must make the derivation fail, with `drift.log` carrying both the comparator's ERROR string and the runtime line of the diff. Sabotages, each applied alone: drop `runtime` from the projection expression, and make the fixture identical to the real facts; both must make this invocation stop failing. Then three **wrong-reason** fixtures, each leaving `runtime` untouched, which must each make the check go red rather than pass: a name-set mismatch (an extra machine key), a malformed candidate fixture that breaks the candidate-side `jq`, and drift on `ip` alone. Passing on any of those three means the pin proves only "the comparator can fail", not "the comparator compares runtime".
