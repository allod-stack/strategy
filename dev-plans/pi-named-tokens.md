...
The command-backed selector has two modes: resolve a credential by startup precedence, or resolve one explicit provisioned token name. It validates exactly one RFC 6750 line and writes the bearer only to Pi's stdout credential pipe. Safe credential/token/catalogue identifiers may appear in its argv; the bearer may not. Measured Pi 0.84.2 provider composition prefers a stored `auth.json` credential over any provider-record or extension key, so the startup selector command lives in the managed `models.json` provider record, where supported re-registration can override it; `auth.json` gains no new managed entries, its recorded legacy entries retire through the ownership transition, and an operator credential appearing afterward under a managed provider ID is a named, preserving refusal. The extension selects and re-registers provider authentication before the first request; the startup picker runs detached (Pi cannot await UI inside `session_start` under RPC) behind a gate that lets no request authenticate an unselected credential.
...
Reconciliation is a whole-operation no-op when any desired Age token is absent. Because auth commands address volatile paths directly, the runtime ownership manifest stays at its current version and needs no per-token link records. The reconciler accepts a prior manifest only after its exact shape, provider list, credential-link paths, and derived targets pass the existing strict validation. Each recorded legacy path must then be either absent (an interrupted retirement already removed it) or a symlink to its exact recorded target; any other occupant is corrupt and causes whole-operation refusal with that occupant preserved. The strictly validated prior manifest is the complete and only ownership proof for retirement: through the existing transaction the reconciler retires every recorded legacy per-credential link — regardless of current desired status, since no credential wants a link any more — and records `credentialLinks` as empty. Ownership is never augmented by scanning the filesystem, and a path or exact-target coincidence is never proof, so an operator-created legacy-root symlink survives even when its name and target mimic the old convention.

These files are not one filesystem transaction, so publication order is load-bearing: publish `auth.json` (carrying only managed-entry retirement) and `models.json` (whose provider records address already-validated volatile paths) as separate atomic renames, retire each still-present exact recorded legacy link, then atomically publish the manifest with empty `credentialLinks`. No new runtime link is ever created, so there is no absent-name interval or symlink-to-directory type change. Until the final manifest rename, every retirement target remains durably recorded; a kill after an unlink therefore retries from the same bounded ownership set, treating an absent recorded path as already retired while refusing any foreign occupant. Ordinary trapped failures restore the snapshotted files and exact pre-run links under the existing transaction. Once the empty-link manifest lands, every recorded legacy path is already absent and no cleanup proof has been discarded. An untrapped kill anywhere between the auth/models renames and the manifest rename also leaves any provider that activation newly desires published without an ownership record; the next activation refuses that provider by name under the standing first-activation collision guard, changes no file, and is repaired by deleting the reported entry and rebuilding. Ownership is never inferred from a matching entry value, so that guard is not relaxed, and a provider the prior manifest already records is unaffected, so the legacy-link retirement transition itself converges from every boundary. No durable conversion journal, pre-unlink sync, special SIGKILL restore, transitional ownership map, distinct link root, manifest-version migration, directory-wide sweep, or desired-set-keyed retirement is introduced. A legacy path occupied by an unowned or conflicting object is preserved and reported, never recursively removed. Rebuild, reboot, idempotence, partial token/provider removal, final empty state, extension collision, genuinely corrupt preference/ownership state, and rollback preserve unrelated Pi configuration. Generated auth, catalogue, extension, activation, runtime files, and closures contain no bearer.
...
The secrets witness covers multiple tokens/default-null, exact projection and recipients, missing/duplicate/invalid tokens, bad defaults, missing ciphertexts, absence of any `rotationStrategy` field, and schema sabotage. The Nexus witness covers migration without bearer input; independent add/rotate/remove/default/list; no activation on add; all-token retarget; shared-provider impact; last/default refusal; provider retirement; token-directory creation and empty-only pruning across migrate, rollback/recover, and retire, with last-token remove witnessed as a refusal that preserves the ciphertext and directory, including a populated-directory race proving unrelated contents are preserved rather than recursively removed; dry-run; interrupted migration/mutation and recovery; and child argv/environment/files/output leak scans using runtime-generated synthetic bearers.

The archetypes witness inspects generated activation and extension artifacts and uses synthetic providers plus a local server. It proves both tokens authenticate only with their expected bearer; startup precedence, stale state, interactive ambiguity, and non-interactive refusal; `/token` immediate switching; active-plus-queued deferral, latest-wins, cancel without changing the remembered choice, and persistence only after activation; all shared providers switch together; separate credentials do not; simultaneous processes stay independently pinned; selection-file mode/concurrency; Pi request-time command resolution and agent-settled registration; generated selector commands address fixed volatile per-token paths directly with no managed runtime symlink layer; a valid prior ownership manifest (one still recording legacy per-credential links) retires every recorded legacy per-credential link before publishing `credentialLinks` as empty, idempotently and regardless of current desired status; an absent recorded path resumes as already retired, while a regular file, directory, or wrong-target symlink at that path refuses safely and is preserved; an unrecorded operator-created symlink in the legacy link root survives even when its valid name and exact computed target mimic the old convention. Kill/reboot injection after each separate auth/models rename, every recorded legacy unlink, and the final manifest rename converges idempotently for every provider the prior manifest already records, never publishes auth referencing an absent volatile path, never discards ownership proof before its recorded links are absent, and preserves/reports a foreign occupant; a kill in that same window during an activation that newly desires a provider instead makes the next activation refuse that provider by name without changing a file, and deleting the reported entry and rebuilding converges while the still-unpublished manifest keeps driving legacy retirement; ordinary failure injection restores exact pre-run files and links. No conversion journal, transitional map, distinct link root, manifest-version migration, or runtime directory pruning is present. The witness also covers reconciliation/removal/absence/collision/reboot/GC behavior and proves no bearer enters persistent home, diagnostics, generated files, process argv/environment, observations, or store closures. Event/command tests must execute the generated extension through Pi's supported extension surface rather than testing a rewritten copy. Each witness includes a deliberate sabotage that its comparator rejects.
...
If `pi-provider` is interrupted, run `pi-provider recover` before touching either data checkout. Before private migration, rollback by retaining old pins and layout. After migration, do not downgrade one side: restore the pre-migration profiles/secrets commits together with old deploy pins, or fix forward with the new command and rebuild. A runtime activation failure rolls back its ordinary files and exact pre-run links; after a kill, the next activation completes any interrupted legacy-link retirement from the prior manifest, which remains unchanged until every recorded path is absent and treats an already-absent recorded path as retired. Repair conflicting inputs — including a provider entry that an untrapped kill before the manifest rename left published but unrecorded, which the next activation reports by name as existing outside Allod ownership — and rebuild. Remote token revocation is human-owned and occurs only after fresh and live-switch verification in the private rollout.
