...
Reconciliation is a whole-operation no-op when any desired Age token is absent. The runtime ownership manifest gains a new version for per-token links. The reconciler accepts a prior-version manifest only after its exact old shape, provider list, credential-link paths, and derived targets pass the old strict validation, and before mutation it requires every recorded legacy link still to be a symlink to that exact target. Missing, replaced, malformed, or path-divergent legacy state is corrupt and causes the usual whole-operation refusal. A valid old manifest supplies only the ownership proof needed to retire those exact legacy credential-level links; new per-token links and the new manifest are derived solely from the current desired token set, with collision checks and rollback snapshots covering creation of the new links. Because the new per-token links live under a root distinct from the legacy per-credential leaf, the upgrade is not a symlink-to-directory type change at one path and has no absent-name interval: the reconciler creates and verifies every new nested link first (an additive, idempotent step snapshotted for rollback), then publishes `auth.json`, `models.json`, and the new-version manifest in that order as separate atomic renames, and only then retires every remaining legacy per-credential symlink and prunes the emptied legacy directory with empty-only `rmdir`. This retirement cannot key off the old manifest's `credentialLinks` list the way the current reconciler's no-longer-desired-link retirement does, because that list lives only in the pre-mutation manifest snapshot and is gone once the new-version manifest is committed: a credential retired in the very operation that also performs the version bump would never appear in any future desired-credential set, so a list keyed on "currently desired credentials" could never find it again. Legacy retirement is therefore a standing, credential-set-independent sweep of the legacy root's direct entries — for every entry whose name is a syntactically valid credential id, retiring it exactly when it is a symlink to that same name's computed runtime target (`<runtime>/pi-provider-credential-<name>`) and reporting/preserving anything else unchanged — that the new-version reconciler code runs unconditionally on every activation, not only right after a version transition. That makes retirement self-healing from the directory alone: it needs no persisted list of which credentials used to have a legacy link, so it still converges when a crash separates the manifest commit from retirement, and it still converges when a credential is fully removed in the same operation that bumps the manifest version. These files are not one filesystem transaction: the ordering must ensure that any published auth entry already has its new link, and rollback/retry must handle a kill between every link creation and file rename. A crash or reboot at any point leaves a crash-safe state: before the manifest commit the prior-version manifest and its legacy links are intact and the additive new links are recreated idempotently on retry; after the manifest commit the new layout is already usable and every leftover legacy symlink is retired idempotently by the next reconciliation's unconditional sweep, on the new-version manifest, because a symlink still pointing at its exact computed runtime target is provably the managed legacy link regardless of whether that credential remains desired. No durable conversion journal, pre-unlink sync, restore-exact-symlink recovery, or same-path collision handling is needed. A legacy path occupied by anything other than the exact recorded symlink is preserved and reported as a collision, never recursively removed. Rebuild, reboot, idempotence, partial token/provider removal, final empty state, extension/link collision, genuinely corrupt (not merely prior-version) preference/ownership state, and rollback preserve unrelated Pi configuration. Generated auth, catalogue, extension, activation, runtime files, and closures contain no bearer.
...
The archetypes witness inspects generated activation and extension artifacts and uses synthetic providers plus a local server. It proves both tokens authenticate only with their expected bearer; startup precedence, stale state, interactive ambiguity, and non-interactive refusal; `/token` immediate switching; active-plus-queued deferral, latest-wins, cancel without changing the remembered choice, and persistence only after activation; all shared providers switch together; separate credentials do not; simultaneous processes stay independently pinned; selection-file mode/concurrency; Pi command-cache separation; a valid prior-version ownership manifest upgrades to distinct-root per-token links and idempotently retires the exact legacy per-credential symlink via the unconditional directory sweep, pruning its emptied directory; a credential fully retired in the same operation as the version bump still has its legacy link swept and removed even though it is absent from every subsequent desired-credential set; kill/reboot injection after every link creation and each separate auth/models/manifest rename converges idempotently with no durable conversion journal and no persisted legacy-link list, never publishes auth that points at an absent new link, stranding no link or leaving both layouts absent, while a legacy or new path occupied by a foreign object is preserved and reported; malformed, missing, replaced, or path-divergent legacy links and new-link collisions refuse without mutation; reconciliation/removal/absence/collision/reboot/GC behavior; and no bearer in persistent home, diagnostics, generated files, process argv/environment, observations, or store closures. Event/command tests must execute the generated extension through Pi's supported extension surface rather than testing a rewritten copy. Each witness includes a deliberate sabotage that its comparator rejects.
