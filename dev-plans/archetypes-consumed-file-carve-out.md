...
- `modules/agent-hooks.nix:11` and the `allod-tools` input generally. The tools input contains no ciphertext, so removing its source root is derivation-churn work rather than this issue's exposure boundary. It is not a one-line ride-along: `modules/dev-home-shared.nix:25` also embeds `${allod-tools}/pm`, so the tools root cannot leave the closure until that directory consumer is carved too. Do that, if still useful, as a separate change with its own closure claim.
