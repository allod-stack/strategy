...
**It is a real-root system unit, deliberately, and not an initrd one.** Measured on the pinned systemd 258.7: PID 1 runs `import_credentials()` from `initialize_runtime()` (`src/core/main.c:2453`) before any unit starts, reading `/sys/firmware/qemu_fw_cfg/by_name/opt/io.systemd.credentials` (`src/core/import-creds.c:375`) into `/run/credentials/@system` (`src/shared/creds-util.h:33`). In the *initrd* that import runs before `systemd-modules-load.service` has loaded `qemu_fw_cfg`, so it finds nothing and `$CREDENTIALS_DIRECTORY` is never set; the import that works is the real-root pass, which `--switched-root` re-enables. Contract 5's `qemu_fw_cfg` in `boot.initrd.kernelModules` is what makes the module present by then. So no consumer in this slice runs in the initrd, and any future one must read the sysfs path itself rather than `$CREDENTIALS_DIRECTORY`.

- **Input.** `LoadCredential = <name>` for each declared name, with no source path, so systemd resolves it from the system credential store PID 1 populated from fw_cfg. The script reads `$CREDENTIALS_DIRECTORY/<name>` — for a system service that is `/run/credentials/allod-microvm-credentials.service/<name>`, mode `0400`, made readable to the unit's user by ACL. Nothing in the guest ever references a `/run/allod/microvm/...` host path; those strings exist only in QEMU's argv.
- **Validation is mandatory, not defensive.** `systemd.exec(5)` makes a missing `LoadCredential=` with no source path **non-fatal**: the unit starts and the file is simply absent. Only the `…Encrypted=` variants fail the unit. So without explicit validation the materializer would run to completion having written nothing, and sshd would then fail for a reason two units away from the cause. The host's launcher does refuse a missing, non-regular or empty *source* before QEMU starts (`allod/nexus` `nix/microvm/launcher.nix:285-287`), but the guest cannot assume it: upstream's `microvm -r <name>` execs the runner with no unit and no preparation at all, which `allod/nexus` `docs/microvm-host.md:44-48` documents as an unsupported but reachable start path.
...
- **Atomic writes.** Write to a temporary file in the destination directory, then `install`-style rename. `install -m <mode> -o <owner> -g <group>` writes through a temporary and renames, which is what `modules/netrc.nix:39-44` and `allod/nexus` `nix/microvm/launcher.nix:290` both already use; use the same idiom rather than inventing one.
- **Ordering.** `after = [ "local-fs.target" ]`; `before = [ "sshd.service" "nix-daemon.service" "systemd-user-sessions.service" ]`; `wantedBy = [ "multi-user.target" ]`. `systemd-user-sessions.service` is the gate every interactive login waits on, so ordering before it is what "before every consuming user session" reduces to. Additionally `systemd.services.sshd.requires` gains the materializer, so a failed materializer leaves sshd unstarted with a dependency failure rather than started against a missing file — contract 11's stated outcome, made a real edge rather than a coincidence.
...
- Line 846 builds the `HostKey` directives solely from `cfg.hostKeys`. With `hostKeys = []` no `HostKey` line is rendered and sshd falls back to its compiled-in `/etc/ssh/ssh_host_*_key` defaults, which do not exist. So `hostKeys = []` alone gives a guest whose sshd never starts.
- Lines 779-802 define `services.sshd-keygen` with `ConditionFileNotEmpty = map (k: "|!${k.path}") cfg.hostKeys` and `script = concatMapStrings … cfg.hostKeys`. With `hostKeys = []` the condition list is empty, so the unit runs, and the script is the empty string, so it generates nothing. Keeping `hostKeys` pointed at the runtime path instead would leave the condition armed and `sshd-keygen` would mint a fresh ed25519 key the moment the credential failed to arrive — which is the exact failure contract 11 forbids.
- Line 826 defines the module's own `extraConfig` contribution with `lib.mkOrder 0`, so a plain `services.openssh.extraConfig` definition is appended after it.

So for microvm: `services.openssh.hostKeys` defined as `[]` lexically inside `sharedModules` (not `mkForce`; the definition is the builder's own and is simply omitted for microvm), plus `services.openssh.extraConfig = "HostKey <root>/ssh-host-key"`. The check reads the rendered `sshd_config` for exactly one `HostKey` line pointing under the root, and reads the rendered `sshd-keygen.service` script for emptiness.

sshd requires a host key file to be unreadable by group and other; the table's `0600 root:root` satisfies it. The public half is not delivered: sshd derives the public key from the private one, and the pinned module's build-time `sshd -G -T` config check does not open host key files — proved today by `/etc/ssh/<name>`, which does not exist at build time either.
...
13. **The rendered materializer unit.** Build `config.systemd.units."allod-microvm-credentials.service".unit` and read the fragment file. Assert one `LoadCredential=` per declared name; `Before=` naming `sshd.service`, `nix-daemon.service` and `systemd-user-sessions.service`; `RemainAfterExit=yes`. Paired sabotage: a fixture that drops the `before` list must lose those needles.
...
15. **The materializer script has no optional branch.** Write the script to a store file and assert `missing or empty, skipping` is absent, and that each declared credential name appears in a validation command. Paired positive control: the same needle must still be *present* in `machineConfigurations.allod-dev`'s `system.activationScripts.netrc.text`, which is what the existing `netrc-activation` check reads — so the two checks together prove the branch was removed from one path and kept on the other.
16. **The rendered `sshd_config`.** Read `config.environment.etc."ssh/sshd_config".source` for a microvm fixture. Assert exactly one `HostKey` line and that it points under the credential root. Paired sabotage: a fixture whose `extraConfig` is forced empty must have zero `HostKey` lines.
17. **The rendered `sshd-keygen.service` script is empty** for a microvm fixture and non-empty for `allod-dev`. This is the anti-regeneration proof; the option read `hostKeys == []` is not.
