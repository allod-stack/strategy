...
Four, all recorded rather than worked around.
...
- `/etc/gitconfig`: append with `lib.mkAfter` on `environment.etc."gitconfig".text`, which is `types.lines`. The appended block is a `[credential]` section header, then an empty `helper =`, then `helper = store --file=<root>/git-credentials`. Not `mkForce`: forcing a `lines` option discards every other definition, and the reset is precisely the mechanism that makes discarding unnecessary. Acceptance test 18 records the same hazard for `nix.extraOptions`, where the original draft's defence against it did not work.
...
- **Ordering: `Before=sysinit.target` with `DefaultDependencies=no`.** This is the single ordering that precedes *every* normal service, every socket unit, `systemd-user-sessions.service`, `user@.service` and `sshd.service`, because systemd gives all of them a default `After=sysinit.target` (`systemd.service(5)`, `systemd.socket(5)`). Enumerating consumers instead — `before = [ "sshd.service" "nix-daemon.service" … ]` — is a list that goes stale the moment a profile adds a reader, which on a machine whose whole point is composed profiles is not a hypothetical. The skeleton is the one `nixos/modules/security/wrappers/default.nix:305-320` already uses for the same problem, plus the reactivation edges:
  ...
  **Both edges are symlinks, not unit-file lines, so a check has to read the right artifact.** Pinned `nixos/lib/systemd-lib.nix` renders `wantedBy`, `upheldBy` and `requiredBy` as `.wants`, `.upholds` and `.requires` symlinks — its own comment is *"Create .wants, .upholds and .requires symlinks from the wantedBy, upheldBy and requiredBy options"* — emitted by `generateUnits` into the tree that lands at `config.environment.etc."systemd/system".source`. Neither materializer's fragment gains a `WantedBy=` line from this, and `sysinit-reactivation.target`'s fragment never gains a `Requires=` line naming them. Acceptance test 13 asserts the symlinks for that reason.
...
- **Load each surviving fixture up.** One bad `credentialFiles` map can carry several invalid keys *and* several invalid values at once; one map can be both missing a required key and carrying an extra one; the generated-unit mutations of tests 13 to 15 can share a fixture, since `checkSabotage` already takes a list of needles per fixture for precisely this reason. Combine wherever the needles stay individually pinned.
  ...
  Two mutations cannot join that shared fixture and each needs its own thunk: test 13's `wantedBy = []` sabotage, whose entire point is that every *other* needle stays green while one symlink disappears, and test 23's credential-copy mutation, which has to evaluate far enough to render the artifacts the scanner reads. Test 5's merge hands one forced dev system back; these two spend it and one more. That is what the per-batch measurement below is for.
...
13. **The two rendered materializer units, and the split itself.** Build `config.systemd.units."allod-microvm-host-key.service".unit` and `…"allod-microvm-forge-credentials.service".unit` and read both fragment files. Assert:
    ...
    - **The four pull symlinks**, read off `config.environment.etc."systemd/system".source` and not off any fragment: `sysinit.target.wants/allod-microvm-host-key.service`, `sysinit.target.wants/allod-microvm-forge-credentials.service`, `sysinit-reactivation.target.requires/allod-microvm-host-key.service`, and `sysinit-reactivation.target.requires/allod-microvm-forge-credentials.service`. Contract 6 gives the mechanism: `wantedBy`/`requiredBy` render as symlinks, so `sysinit-reactivation.target`'s own unit file carries no `Requires=` line naming these units and an assertion looking for one fails a *correct* implementation. The `sysinit.target.wants` pair is the only thing in this plan that asserts either materializer starts at boot at all, and for the Forge unit it is the only start path that exists.
...
14. **sshd's real dependency edge, and the edge that must not exist.** In the rendered `sshd.service`, assert `Requires=` names `allod-microvm-host-key.service` and does **not** name `allod-microvm-forge-credentials.service`. Assert no unit anywhere in the rendered configuration carries a `Requires=` or `Requisite=` on the Forge unit. Paired sabotages: dropping the `requires` definition loses the first needle; adding the Forge unit to sshd's `requires` must fail the second.
    ...
    Every fixture here runs on the default `!startWhenNeeded` branch, where nixpkgs renders `sshd.service` and `sshd@.service` and no `sshd.socket`, so this test reads `sshd.service`. **No fixture may assert that an `sshd@` attribute is absent** — contract 6 measures why: nixpkgs defines that template unconditionally, so the assertion would fail on a correct implementation. The socket branch carries no fixture at all, because nothing in Allod sets the option and a fixture that set it would be testing nixpkgs.
...
19. **Both rendered Git config files.** Contract 4 sets out the design; this test reads both artifacts in their own rendering.
    - `/etc/gitconfig`: read `config.environment.etc."gitconfig".text`. Assert the inherited `helper = store` is still present (nothing was forced away), that the appended block opens with its own `[credential]` header, that a bare `helper =` reset appears after that header, and that the *last* helper line is `helper = store --file=<root>/git-credentials`, unquoted. Assert the header **by section, not by presence**: the nearest preceding `[…]` line before the reset must be `[credential]`. A presence check is satisfied by the exact failure contract 4 describes — a third definition landing between the two blocks and re-homing both appended lines into some other section.

      That section assertion carries **no fixture**, and the plan does not claim one. The only way to reach its failure is a third `environment.etc."gitconfig".text` definition, which means another forced dev system for a header the implementation always emits, and this check's memory budget does not buy that. It is an anchor against a future change, not a proved-failing check; contract 3 records the same shape for the value-side absolute rule.
...
23. **No consumer path under a persistent mount, read off generated artifacts.** Scan both materializer scripts, the rendered `sshd_config`, `nix.conf`, both Git config files and the rendered `~/.ssh/config` for any literal equal to or under a declared `microvm.volumes` mount point. This is the scanner contract 14 asks for, and it is what catches a copy command as well as a destination.

    **It needs one paired sabotage, and today it has none.** This is an absence scan: delete the whole scanner and every fixture stays green. Test 9 does not cover it — that fixture sabotages the contract-14 *assertion*, so it fails at evaluation and the scanner never runs against the artifacts at all. `architecture.md` principle 11 and the parent's validator-validation rule are the same rule, and test 22 obeys it with two synthetic canaries while this test does not obey it at all. The cheap fix rides a fixture that already exists: extend the unit-mutation fixture tests 13 to 15 share with one line in the Forge unit — an `ExecStartPost` or `script` line copying a materialized credential under the dev home mount (`/home/<user>/…`, which the volumes module declares as a `microvm.volumes` mountPoint) — and require the scanner to name that literal. No new forced system, and the mutation is exactly the class the scanner exists for: a *copy* whose destination is durable, not a mis-set option.
...
25. `dev-forge-opt-out` is unchanged — its `withTokens` helper and its literal `expectedWithAccess` projection for the libvirt `allod-dev` both stay as they are. Test 2's microvm cases are built from `runtime-module-selection`'s `devFixture` rather than by widening this check or sharing its helper across two `let` scopes, so the libvirt contract stays pinned to a literal and each check keeps its own fixtures.
