...
Home Manager installs one managed global Pi extension only when the provider set is non-empty. It registers a complete Pi provider with both OpenAI stream adapters as needed. Its auth `check` reports `Age-managed VM credential` without reading or returning the value; `resolve` reads the one fixed runtime path and ignores any stored `auth.json` credential or `models.json` key for that provider. A missing, empty, or newline-bearing runtime file fails the request loudly. Existing unrelated Pi providers/configuration remain untouched.
