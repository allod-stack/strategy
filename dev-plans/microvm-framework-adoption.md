...
9. Stop the microvm through the upstream QMP `ExecStop`, assert its prepared host credential directory and QMP working directory are removed only after QEMU exits while its volume remains, then prove manual restart, automatic retry, rebuild stop, and rollback restart all use a newly reachable QMP socket.
...
- The root launch helper prepares credentials on every start, invokes the matching runner only after preparation, gives the root-namespace upstream `ExecStop` the same per-VM QMP socket path that QEMU sees in its namespace, cleans up the credential and QMP directories only after QEMU exits and every upstream `ExecStopPost` completes, and puts no secret bytes in its argv, environment, or logs.
