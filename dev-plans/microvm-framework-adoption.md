...
9. Realise or add a path into the guest's Nix store, restart the microvm host unit, then change the guest toplevel used by the generated restart trigger and restart again. Assert the path remains valid with `nix-store --verify-path` or `nix path-info`, its contents survive, the nix daemon is running, `/nix/var/nix` is a declared `neededForBoot` persistent volume, and `/nix/store` is an overlay whose upper/work directories are under `/nix/var/nix`. Paired fixtures must prove `writableStoreOverlay = null` fails the write against the read-only erofs mount; an overlay backing persisted outside `/nix/var/nix` can leave the file in `/nix/store` but fails Nix validity after reboot because the database was lost; and a missing, unformatted, or mislabeled `/nix/var/nix` volume fails in the initrd before writers can use the tmpfs directory underneath.
...
- Selected dev microvm with a null `writableStoreOverlay`, a value other than `/nix/var/nix`, no declared persistent `/nix/var/nix` volume, `microvm.registerClosure = false`, or store auto-optimisation enabled.
...
- Each selected dev microvm has `microvm.writableStoreOverlay = "/nix/var/nix"`, `/nix/var/nix` as a declared `neededForBoot` persistent volume, `/nix/store` overlay upper/work below that volume, `microvm.registerClosure = true`, nix daemon and socket enabled, and store auto-optimisation disabled.
...
For a private guest that has already run, stop microvm autostart and return the inventory selection to libvirt only after relaying or pushing all work from every declared persistent volume. For dev microvms, `/nix/var/nix` is part of that authoritative set; preserving only home or only the store overlay files is not a valid relay. A revert reconstructs the root and store image but cannot reconstruct unique files on a volume.
