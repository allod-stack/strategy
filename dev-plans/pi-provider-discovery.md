...
- `supported_endpoints` may identify Chat Completions or Responses. Provider-level `api` is never discovered: it stays `openai-completions` unless `--api` names another adapter. Without explicit `--api`, a model that explicitly supports Responses and not Chat Completions carries model-level `api: openai-responses`, which Pi honors over the provider value; every other model carries none. Explicit `--api` suppresses model-level overrides because the operator selected one adapter for the whole provider.
