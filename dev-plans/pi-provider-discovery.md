...
The base URL is a control-character-free HTTPS URL without userinfo, query, or fragment. Discovery performs one authenticated `GET <base-without-trailing-slash>/models`, never follows redirects, never invokes another route, limits response bodies to 1 MiB, and applies connection/overall timeouts. Non-2xx responses fail before mutation; output may include status and at most 512 normalized characters of provider error text after exact bearer redaction.
...
Bearer resolution is: force a hidden prompt with `--prompt-token`; otherwise, for an existing credential, decrypt only its non-null `defaultToken` ciphertext with the Nexus hypervisor Age identity (`AGE_IDENTITY`, default `$HOME/.ssh/host`, as in `rotate-token`) into a shell variable, never a file; on null default, missing ciphertext, missing or non-recipient identity, or decryption failure, explain and use the same hidden prompt. A fallback bearer is request-only and changes no token state. A new credential prompts once, uses that value for discovery, and encrypts it as `default` only after discovery and all staging preflights pass. Plaintext is held only in a non-exported shell variable, cleared after use, and enters Age/curl through builtin-printf pipes—not argv, environment, logs, diagnostics, shell tracing, regular files, or journals. Dry-run still performs this authenticated read and may decrypt/prompt, but never encrypts or mutates.
...
- `supported_endpoints`, `supports_chat_completions`, `supports_responses`, `capabilities.chat_completions`, and `capabilities.responses` may identify Chat Completions or Responses. Chat wins when both are explicit. If every model is explicitly Responses-only, provider `api` is `openai-responses`; otherwise it is `openai-completions`, with `api: openai-responses` on individual Responses-only models. Explicit `--api` suppresses discovered adapter overrides.
...
`discover-refresh` resolves the provider's current credential and URL, fetches/normalizes a catalogue, and prints sorted `Added`, `Removed`, and `Changed` ID groups. It changes only that provider's translated `models` and discovered API metadata; credential targets, named tokens, default selection, URL, and unrelated providers remain byte-semantically unchanged. Equal normalized catalogues return without staging, preserving a no-diff result.

If removals appear, refresh fetches a second complete snapshot and compares normalized persistence-affecting data. Empty/invalid or differing snapshots abort before mutation. An agreeing live run prints the removal diff and accepts only an explicit `yes`; refusal, EOF, or interruption leaves no journal or repository change. Dry-run performs both reads and reports that confirmation would be required without prompting. New/changed metadata needs no confirmation.
...
The black-box witness retains the existing lifecycle suite and adds rich/minimal translation; default/all and explicit targets; adapter/capability precedence; warning-only metadata; malformed, empty, duplicate, oversized, non-2xx, and bounded-error responses; new/shared credentials; encrypted-default reads, missing-identity fallback, and forced prompts; the impact summary preceding every prompt or decryption; token-state preservation; argv/environment/output/file leak scans; stable add/change/remove groups and ordering; agreeing/disagreeing removal snapshots; yes/refusal/EOF/signal behavior; no-diff refresh; rollback/recover at refresh mutation boundaries; and a sabotaged client that rejects any route except the exact `/models` endpoint. Fixtures replace only HTTP/Age inputs at existing executable seams and run the production generator/transaction path.
